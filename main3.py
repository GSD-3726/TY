...
# ############################################################################
# 三层筛选测速配置 (GitHub Actions免费版优化，保证测速精度前提下缩短耗时)
# ############################################################################
ENABLE_FFMPEG = True                     # 是否启用FFmpeg测速
FFMPEG_PATH = "ffmpeg"                   # FFmpeg命令路径
# 第一层：连通性预检（缩短超时，快速筛死链）
CONN_TIMEOUT = 1.8                       # 原3.0 → 1.8s，Actions网络波动可承受，大幅减少等待
CONN_CONCURRENCY = 80                    # 原100 → 80，降低并发避免连接队列阻塞Actions
# 第二层：快速探测（大幅缩短检测时长，6s→3s，阈值小幅收紧保证精度）
FAST_FFMPEG_DURATION = 3                 # 原6秒 → 3秒，缩短一半快速探测耗时
FAST_PROC_TIMEOUT = 7                    # 原12 → 7，进程超时同步缩短
FAST_FFMPEG_CONCURRENCY = 12             # 原30 → 12，Actions 2核CPU最大稳定并发
FAST_MIN_FRAMES = 30                     # 原45 → 30，适配3秒时长，判定标准不变
FAST_MIN_SPEED = 0.8                     # 原0.75小幅收紧，过滤勉强能跑的劣质流，减少进入第三层数量
# 第三层：稳定测试（20s→10s，减半耗时，各项判定阈值微调保证准确率）
STABLE_FFMPEG_DURATION = 10              # 原20秒 → 10秒，核心耗时减半
STABLE_PROC_TIMEOUT = 16                 # 原30 → 16，同步缩短进程超时
STABLE_FFMPEG_CONCURRENCY = 6            # 原15 → 6，匹配2核Linux不OOM
MIN_AVG_FPS = 15                         # 最低平均帧率不变
MIN_FRAMES = 120                         # 原250适配10秒时长，精度等价
MIN_REALTIME_FACTOR = 0.60               # 最低实时因子不变
MIN_NET_FEED_RATIO = 0.65                # 最低网络供给比不变
FFMPEG_RETRIES = 1                       # 预留，暂未使用
# FFmpeg全局低延迟优化参数（新增，所有测速命令统一复用，减少探测耗时）
FFMPEG_LOW_LATENCY_ARGS = [
    "-fflags", "+genpts+nobuffer+discardcorrupt+ignidx+fastseek",
    "-avioflags", "direct",
    "-flags", "low_delay",
    "-threads", "2",          # Actions 2核限制，固定线程避免CPU争抢
    "-rw_timeout", "3000000", # 3s网络读写超时，原8s缩短
    "-analyzeduration", "300000", # 300ms流探测(原1s)，快速识别流信息
    "-probesize", "131072",   # 128KB探测缓冲区(原1M)，减少读取数据量
]
# ############################################################################
# 缓存 配置区域 (可根据需要调整)
# ############################################################################
...
async def quick_connectivity_test(urls: List[str]) -> List[str]:
    ...
    async def _check_one(url: str):
        async with semaphore:
            try:
                timeout = aiohttp.ClientTimeout(total=CONN_TIMEOUT, connect=1.0)
                async with aiohttp.ClientSession(timeout=timeout) as session:
                    async with session.get(url, headers={
                        "User-Agent": "Mozilla/5.0",
                        "Referer": "https://www.miguvideo.com/"
                    }, read_bufsize=512) as resp:
                        if resp.status in (200, 206, 301, 302):
                            chunk = await resp.content.read(256)
                            if chunk and b'<html' not in chunk.lower()[:80]:
                                return url
            except Exception:
                pass
            return None
...
async def stable_ffmpeg_test(url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        startup_offset = 1.0 if elapsed > 2.0 else 0.6
...
async def batch_test_pipeline(channel_map: Dict[Tuple[str, str], List[str]]
    ...
    """三层筛选流水线：连通性->快速FFmpeg->稳定测试 (GitHub Actions优化版)"""
    if not channel_map:
        return {}
    ...
    if not all_urls:
        logger.info(f"全部来自缓存: 有效{cached_ok} 失效跳过{cached_fail}")
