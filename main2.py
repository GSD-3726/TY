...
MIN_NET_FEED_RATIO = 0.60
MIN_LATE_SPEED = 0.70
MIN_SPEED_MIN = 0.60                  # 原0.85太严
MAX_FPS_JITTER = 0.30
FFMPEG_RETRIES = 1

MAX_SPEED_STD = 0.40                  # 原0.20太严
MIN_SPEED_PERCENTILE_20 = 0.60
SPEED_DECAY_THRESHOLD = 0.35
LATE_SEGMENT_MIN_SPEED = 0.65
EARLY_PASS_TIME = 20
EARLY_PASS_MIN_SPEED = 1.05
EARLY_PASS_LATE_SPEED = 0.90
EARLY_PASS_MAX_STD = 0.20
...
STEALTH_JS = """delete Navigator.prototype.webdriver;Object.defineProperty(Navigator.prototype,'webdriver',{value:undefined,writable:false,configurable:true});if(!window.chrome)window.chrome={};window.chrome.runtime={connect:function(){return{onMessage:{addListener:function(){}},postMessage:function(){},onDisconnect:{addListener:function(){}}}},sendMessage:function(){},onConnect:{addListener:function(){},removeListener:function(){},hasListener:function(){return false}},onMessage:{addListener:function(){},removeListener:function(){},hasListener:function(){return false}},getURL:function(p){return'chrome-extension://invalid/'+p},id:undefined};for(let k in window){if(k.startsWith('cdc_')||k.startsWith('__webdriver')||k.startsWith('__driver')||k.startsWith('__selenium'))delete window[k]};const origQuery=window.navigator.permissions.query;window.navigator.permissions.query=(p)=>p.name==='notifications'?Promise.resolve({state:Notification.permission}):origQuery(p);"""
...
async def quick_connectivity_test(urls: List[str]) -> List[str]:
    ...
    async def _check_one(url: str):
        async with semaphore:
            try:
                timeout = aiohttp.ClientTimeout(total=CONN_TIMEOUT, connect=2.0)
...
async def stable_ffmpeg_test(url: str) -> Dict[str, Any]:
    ...
    try:
                ...
                if speed_match:
                    ...
                    if elapsed > 5.0:
                        if speed < MIN_SPEED_MIN:
                            ...
                            if low_speed_duration >= LOW_SPEED_KILL_DURATION:
                                proc.kill()
                                break
                        else:
                            low_speed_duration = 0.0
        ...
        is_ok = (
            ...
            and speed_std <= MAX_SPEED_STD
            and speed_p20 >= MIN_SPEED_PERCENTILE_20
        )
        ...
        return {
            ...
            "speed_std": 999, "decay": 1.0, "min_speed": 0
        }
...
def stream_quality_score(item: tuple) -> float:
    ...
    time_score = max(0.0, 1.0 - (max(elapsed - 4.0, 0) / 8.0))
    if w >= 1920:
        res_score = 1.0
    elif w >= 1280:
        res_score = 0.7
...
async def batch_test_pipeline(channel_map: Dict[Tuple[str, str], List[str]]
    ...
    async def _stable_test(url: str, fast_res: dict):
        await asyncio.sleep(random.uniform(0, 3))  # 错峰启动
    ...
    for coro in asyncio.as_completed(stable_tasks):
        ...
        if res["ok"]:
            ...
            fail += 1
            logger.warning(
                f"FAIL {url[:60]} | fps={res['fps']} speed={res['speed']} "
                f"late={res['late_speed']} min={res['min_speed']} "
                f"std={res['speed_std']} decay={res['decay']} "
                f"frames={res['frames']} jitter={res['fps_jitter']} "
                f"errors={res['has_errors']}"
            )
...
async def extract_detail_channels_playwright(ctx, detail_url: str) -> list:
    ...
    try:
        ...
        s_link = await page.evaluate(r""" () => { const links = document.querySelectorAll('a[href*="?s="]'); for (const a of links) { const href = a.getAttribute('href') || ''; if (href.includes('?s=')) { return href; } } return null; } """)
        if s_link:
        ...
        for page_num in range(1, MAX_DETAIL_PAGES + 1):
            ...
            page_channels = await page.evaluate(r""" () => { const results = []; const rows = document.querySelectorAll('table.iptv-table tbody tr, table tbody tr'); for (const row of rows) { const cells = row.querySelectorAll('td'); if (cells.length < 3) continue; const name = cells[1] ? cells[1].innerText.trim() : ''; let url = ''; const a = cells[2] ? cells[2].querySelector('a') : null; if (a) { url = a.getAttribute('href') || a.innerText.trim(); } else if (cells[2]) { url = cells[2].innerText.trim(); } if (name && url) { results.push({name: name, url: url}); } } return results; } """)
...
# ========================== 主流程 ==========================
async def main():
    parser = argparse.ArgumentParser(description="IPTV源抓取器")
    parser.add_argument("--type", default="all", help="all/hotel/multicast/migu/other")
    ...
    ft = norm_type(args.type) if args.type.strip().lower() != "all" else norm_type(SCRAPE_SOURCE_FILTER)
            ...
            if not chrome_path:
                ...
                for c in candidates:
                    if Path(c).exists() and Path(c).is_file():
                        chrome_path = c
                        break
            logger.info(f"Chrome路径: {chrome_path if chrome_path else 'Playwright默认'}")
    ...
    total_before = sum(len(v) for v in ch_map.values())
