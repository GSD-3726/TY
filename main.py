...
MAX_IPS = 30                                     # 【数字】最多处理前N个IP/地址行 (0表示不限制)
...
FFMPEG_CONCURRENCY = 5                            # 【数字】同时测试的链接数 (GitHub Actions建议<=2，本地电脑建议<=CPU核心数)
...
async def run_ffmpeg_test(channel_map: Dict[Tuple[str, str], List[str]]) -> Dict[Tuple[str, str], List[str]]:
    ...
    for (group, name), urls in channel_map.items():
        ...
        if group in TEST_ONLY_GROUPS:
            ...
            # 不在白名单的直接保留（本代码已过滤，这里不会走到）
            for url in urls:
                result_map[(group, name)].append((url, 0.0, 0, 0))
...
def finalize_results(result_map):
    ...
    logger.info(f"测速筛选完成，最终保留 {total_final} 条优质频道链接")
    return final_map
