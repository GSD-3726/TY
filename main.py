        ...
        if existing_channels:
            ...
            if missing_groups:
                ...
                for group, names in required_channels.items():
                    for name in names:
                        if (group, name) not in existing_set:
                            required_map[(group, name)] = MAX_LINKS_PER_CHANNEL
                            missing_channels.append(f"{group}-{name}")
                if missing_channels:
                    logger.info(f"检测到 {len(missing_channels)} 个要求频道在旧文件中缺失，将进行全量更新: {missing_channels[:10]}...")

                # 处理旧文件中的频道（连通性测试 + 需求计算）
                all_old_urls_list = []
                ...
                for (g, n), urls in existing_channels.items():
                    # 如果该频道已通过 required_map 设置为全量更新（即频道缺失但后来新源补充），则仍使用全量需求
                    if (g, n) in required_map:
                        continue  # 需求已在缺失处理中设为 MAX_LINKS_PER_CHANNEL
                    valid_urls = []
                    for u in urls:
