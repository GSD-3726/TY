...
# -------------------------- 2.5 分批测速设置 (新增) -------------------------
TEST_FRACTION         = 0.25                            # 【关键】每次运行测试的比例 (0.0 - 1.0)
                                                        # 例如: 0.25 = 每次测1/4, 0.5 = 测一半, 1.0 = 全测
BATCH_SEED            = "batch_v1"                      # 随机种子，改变这个值可以重新打乱分批顺序
...
# -------------------------- 6. 缓存设置 ------------------------------------
ENABLE_CACHE           = True                            # 启用测速缓存 (必须开启，否则分批无意义)
CACHE_FILE             = OUTPUT_DIR / "iptv_speed_cache.json"
CACHE_EXPIRE_HOURS     = 168                             # 缓存过期小时 (建议设长一点，例如168小时=7天)
...
CATEGORY_RULES = [
    ...
    {"name": "央视频道",    "keywords": ["cctv", "cetv", "央视", "中央", "CCTV", "CETV", "新闻", "综合", "财经", "综艺", "体育", "电影", "军事", "电视剧", "纪录", "科教", "戏曲", "社会与法", "少儿", "音乐", "奥林匹克", "农业农村"]},
...
# ======================== 【新增】分批逻辑核心函数 ============================
# ============================================================================
def is_in_current_batch(url: str, fraction: float, seed: str) -> bool:
    """
    根据URL的哈希值判断它是否属于当前批次
    """
    if fraction >= 1.0:
        return True
    # 组合 URL 和 种子 生成哈希
    hash_input = (seed + url).encode('utf-8')
    hash_val = hashlib.md5(hash_input).hexdigest()
    # 将哈希值转为 0-1 之间的浮点数
    float_val = int(hash_val, 16) / (16**32)
...
async def run_ffmpeg_test(channel_map: Dict[Tuple[str, str], List[str]]) -> Dict[Tuple[str, str], List[str]]:
    ...
    for (g, n), us in channel_map.items():
        for u in us:
            ...
            # 2. 【新增】分批判断：如果不在当前批次，暂不测试
            if not is_in_current_batch(u, TEST_FRACTION, BATCH_SEED):
                skipped_batch += 1
                continue

            pending.append((g, n, u))

    logger.info(f"总链接: {total} | 缓存有效成功: {cached_ok} | 缓存失效/无缓存: {cached_failed_skipped + len(pending) + skipped_batch}")
    logger.info(f"本轮计划测速: {len(pending)} 个 (跳过 {skipped_batch} 个不属于本批次)")

    if not pending:
        logger.info("没有需要在本轮测速的链接，直接返回缓存结果")
        final = {}
    ...
    logger.info(f"测速完成，共 {len(final)} 个频道通过筛选 (包含历史缓存)")
    return final
...
async def main():
    ...
    logger.info(f"✅ 分批测速设置：本次运行 {TEST_FRACTION*100:.0f}% 的链接")
    ...
    web_entries = []
    # 注意：为了演示分批，这里可以把爬取部分稍微简化或跳过，重点在测速
    # 如果你不需要每次都爬取网站，可以把 MAX_IPS 设为 0 或注释掉这一块
    if MAX_IPS > 0:
    ...
    logger.info("--- 正在进行 FFmpeg 测速 (含分批逻辑) ---")
    ...
    logger.info("="*30)
    logger.info(f"⏱️  任务完成。提示：修改配置中的 TEST_FRACTION 并再次运行以测试更多链接。")
    logger.info("="*30)
